...
const ProfilePage = () => {
    ...
    const [customerDataUpdate, { data: cUpdateResponse, loading: cUpdateLoading, error: cUpdateError }] = useMutation(UPDATE_CUSTOMER_MUTATION, {
        variables: {
            input: customerData
        },
        onCompleted: () => {
            // console.warn( 'completed CHECKOUT_MUTATION' );
            cogoToast.success('Customer data updated', {
        ...
        onError: (error) => {
            if (error && error.graphQLErrors) {
                ...
                cogoToast.error('There was an error, try again later', {
                    position: 'bottom-right',
                    hideAfter: 3
                })
            }
                            ...
                            <div className="px-4 lg:px-8">
                                { !userLoading
                                    ...
                                    <div>
                                        ...
                                        <button className="checkout-bttn mx-auto block my-4 relative" style={{ minHeight: "38px" }} onClick={(e) => userComplete(e)}>
                                            {!cUpdateLoading
                                                ?
                                                "Save"
                                                :
                                    ...
                                    <div>
                                        <h2 className="font-bold text-2xl">General</h2>
                                        <div><Skeleton height={38} width={"100%"} /></div>
